...
def train(args, io):
    ####          meta_add_noisy
    train_loader = DataLoader(ShapeNetPart_SP(partition='train', num_points=args.num_points, class_choice=args.class_choice), num_workers=8, batch_size=args.batch_size, shuffle=None, drop_last=False)
    ...
    model.load_state_dict(torch.load('outputs/'+ args.exp_name +'/models/model.t7'))
    ...
    with torch.no_grad():
        ...
        for data, augdata, seg, label, mask, splabel in train_loader:
            ...
            splabel_hot = torch.zeros(len(splabel), num_sp).scatter(1, splabel.view(-1, 1), 1)
            ...
            logits_feat, logits_Augfeat = mix_feat.chunk(2)
            logits_feat = logits_feat.permute(0, 2, 1).contiguous()
            logits_Augfeat = logits_Augfeat.permute(0, 2, 1).contiguous()
            ...
            train_feat.append(logits_feat.detach().cpu().numpy())
            train_prob.append(F.softmax(logits_X, dim=-1).detach().cpu().numpy())
        ...
        outstr = 'Test, loss: %.6f, train acc: %.6f, train avg acc: %.6f, train iou: %.6f' % (train_loss*1.0/count,
                                                                                                  train_acc,
                                                                                                  avg_per_class_acc,
                                                                                                  np.mean(train_ious))
        io.cprint(outstr)
        ...
        np.save('outputs/'+ args.exp_name +'/models/train_feat.npy', train_feat)
        np.save('outputs/'+ args.exp_name +'/models/train_prob.npy', train_prob)
