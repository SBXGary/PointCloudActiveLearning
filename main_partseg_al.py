...
from data import ShapeNetPart, ShapeNetPart_SP
...
def train(args, io):
    ####          meta_add_noisy
    train_data = ShapeNetPart_SP(partition='train', num_points=args.num_points, class_choice=args.class_choice)
    train_loader = DataLoader(train_data, num_workers=8, batch_size=args.batch_size, shuffle=True, drop_last=True)
    val_loader = DataLoader(ShapeNetPart_SP(partition='val', num_points=args.num_points, class_choice=args.class_choice), num_workers=8, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    test_loader = DataLoader(ShapeNetPart_SP(partition='test', num_points=args.num_points, class_choice=args.class_choice), num_workers=8, batch_size=args.test_batch_size, shuffle=True, drop_last=False)
    
    np.save('outputs/'+ args.exp_name +'/models/sp_mask.npy', train_data.sp_mask)
    ...
    # model.load_state_dict(torch.load('outputs/exp/models/model.t7'))
    ...
    for epoch in range(args.epochs):
        ...
        train_pred_seg = []
        train_label_seg = []
        for data, augdata, seg, label, mask, splabel in train_loader:
            ...
            ### sp label onehot
            splabel = splabel.view(-1)
            splabel_hot = torch.zeros(len(splabel), num_sp).scatter(1, splabel.view(-1, 1), 1).view(data.shape[0], data.shape[1], num_sp)
            data, augdata, label_one_hot, seg, mask, splabel_hot = data.to(device), augdata.to(device), label_one_hot.to(device), seg.to(device), mask.to(device), splabel_hot#.to(device)
            batch_size = data.size()[0]

            #### mixup
            all_inputs = torch.cat([data, augdata], dim=0)
            all_labels = torch.cat([label_one_hot, label_one_hot], dim=0)

            opt.zero_grad()
            mix_logits, mix_feat = model(all_inputs.permute(0, 2, 1), all_labels)
            logits_X, logits_Aug = mix_logits.chunk(2)
            logits_X = logits_X.permute(0, 2, 1).contiguous()
            logits_Aug = logits_Aug.permute(0, 2, 1).contiguous()
            #### logits_Aug = logits_Aug.detach()

            #### nuclear loss 
            # logits_X_sp = logits_X.unsqueeze(dim=2).cpu() * splabel_hot.unsqueeze(dim=3) ###  bn1k * bnm1 = bnmk
            # loss_nuc = (torch.sum(torch.linalg.svd(logits_X_sp)[1], dim=-1)).to(device).mean()
            #### final loss
            loss_X = criterion(logits_X.view(-1, seg_num_all), seg.view(-1), mask=mask.reshape(-1, 1))
            loss_pi = 1.0 * ((F.softmax(logits_X.view(-1, seg_num_all), dim=1) - F.softmax(logits_Aug.view(-1, seg_num_all), dim=1)) ** 2).sum(dim=1).mean()

            w = args.lambda_u * linear_rampup(epoch, args.epochs/2.0)
            loss = loss_X + w * loss_pi# + w * loss_nuc
            loss.backward()
            opt.step()

            pred = logits_X.max(dim=2)[1]               # (batch_size, num_points)
            count += batch_size
        ...
        with torch.no_grad():
            ...
            for data, seg, label in test_loader:
                ...
                batch_size = data.size()[0]
                seg_pred, feat = model(data, label_one_hot)
